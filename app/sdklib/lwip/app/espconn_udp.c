...
espconn_udp_sent(void *arg, uint8 *psent, uint16 length)
{
    ...
    struct udp_pcb *upcb = (struct udp_pcb *)pudp_sent->pcommon.pcb;
...
//    struct pbuf *q = NULL;
	...
	if (wifi_get_opmode() != 1) {
		...
		struct ip_addr *ii = (struct ip_addr *)(&precv->pespconn->proto.udp->remote_ip[0]);
		if (!ip_addr_netcmp(ii, &ipconfig.ip, &ipconfig.netmask)) {
