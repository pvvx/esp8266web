...
//-------------------------------------------------------------------------------
// Test adc
// „итает adc в одиночный буфер (~2килобайта) на ~20ksps и сохран€ет в виде WAV
// ѕравильное чтение организуетс€ по прерыванию таймера(!).
// “ут только демо!
//-------------------------------------------------------------------------------
typedef struct
{ // https://ccrma.stanford.edu/courses/422/projects/WaveFormat/
  ...
  unsigned short int bits;/* +34 разр€дность 8/16 */
...
//        	  tcp_puts("%u", system_adc_read());
          ...
          else if(!os_memcmp((void*)cstr, "sleep_old", 9)) tcp_puts("%u",(*((uint32 *)0x60001060))>>16 ); // если нет отдельного питани€ RTC и deep_sleep не устанавливалс€/примен€лс€ при текущем включени€ питани€ чипа, то значение неопределенно (там хлам).
