 ...
 var elapsed;
  ...
  for(i = ajaxList.length; i > 0; i--){
   ...
   if(curAjax.ajaxReq.readyState == 4 && curAjax.ajaxReq.status == 200) {
    ...
    if(curAjax.repeat){
     if(elapsed >= curAjax.repeat) elapsed = 100;
     else elapsed = curAjax.repeat - elapsed;
     setTimeout("newAJAXCommand('"+curAjax.url+"',"+curAjax.container+","+curAjax.repeat+")", elapsed);}
    continue;
   }
   // If we've waited over 4 second, then we timed out
   if((curAjax.ajaxReq.readyState == 4 && curAjax.ajaxReq.status == 404) || (elapsed > 4000)){
    // Invoke the user function with null input
    if(typeof(curAjax.container) == 'function') curAjax.container(null);
    else // Alert the user
     alert("Command failed.\nConnection to device was lost.");
    curAjax.ajaxReq.abort();
    curAjax.ajaxReq = null;
    // If it's a repeatable request, then do so
    if(curAjax.repeat) setTimeout("newAJAXCommand('"+curAjax.url+"',"+curAjax.container+","+curAjax.repeat+")", 200);
    continue;
   }
